...
make_release_fixture() {
  ...
  cp "$PROJECT_ROOT/MANIFEST" "$TEST_REPO/MANIFEST"
  cp "$PROJECT_ROOT/update.sh" "$TEST_REPO/update.sh"
  cp "$PROJECT_ROOT/tests/fakes/curl" "$TEST_REPO/tests/fakes/curl"
  cp "$PROJECT_ROOT/tests/update.bats" "$TEST_REPO/tests/update.bats"
  ...
  for version in 1.1.0 1.2.0; do
    ...
    if [ "$version" = 1.2.0 ]; then
      ...
      printf '%s\n' '# release 1.2.0 marker' >> "$stage_root/once.sh"
...
@test "update replaces only manifest files and preserves project state" {
  ...
  grep -Fq '# release 1.2.0 marker' "$TEST_REPO/once.sh"
...
@test "update aborts while a ralph run lock is present" {
  make_release_fixture
  mkdir -p "$TEST_REPO/.ralph/lock"
  printf '%s\n' 'running pid' > "$TEST_REPO/.ralph/lock/pid"
  before_version="$(cat "$TEST_REPO/VERSION")"
  ...
  [ "$status" -ne 0 ]
  [[ "$output" == *'corrida de ralph en curso'* ]]
  [ "$(cat "$TEST_REPO/VERSION")" = "$before_version" ]
}
