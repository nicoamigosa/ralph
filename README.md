...
- Instalar/actualizar: `ralph/update.sh <VERSION>` descarga esa release,
  ...
  checkpoints y toda `.ralph/`. Mientras `once.sh` corre, `.ralph/lock` impide
  actualizar. El updater nunca crea commits: el diff queda para un PR normal.
...
Después de pasar `bash -n once.sh`, `shellcheck once.sh` y `bats tests/`,
versioná `VERSION`, commiteá y creá el tag `v<VERSION>`. Construí el tarball
desde `MANIFEST` para que no entren archivos fuera de la distribución:
